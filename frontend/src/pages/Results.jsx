...
const generateSummary = (feedback, scores) => {
  ...
  if (errors.length > 0) {
    return `The submitted code contains ${errors.length} critical syntax or logic error${errors.length > 1 ? 's' : ''} that prevent it from running. ${
      warnings.length > 0 ? `Additionally, ${warnings.length} warning${warnings.length > 1 ? 's were' : ' was'} found regarding code quality. ` : ''
...
const CodePanel = ({ code, language }) => (
  <div className="rounded-xl overflow-hidden border border-slate-700/50 bg-[#0d1117]" style={{ height: 420 }}>
    <Editor
      height="100%"
  ...
  return (
    <div className="flex flex-col gap-4 mb-8">
      <div className="flex gap-4 h-[420px]">
        ...
        {/* Output */}
        <div className="flex-1 rounded-xl overflow-hidden border border-slate-700/50 bg-black flex flex-col">
          <div className="bg-slate-800/80 px-4 flex items-center justify-between border-b border-slate-700/50 h-10">
        ...
        {TABS.map(t => {
          ...
          return (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-4 py-2.5 text-sm font-semibold rounded-t-lg transition-colors flex items-center gap-1.5 border-b-2 -mb-px ${
                tab === t
            ...
            >
              ...
              {count != null && count > 0 && (
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ml-0.5 ${
                  tab === t ? 'bg-indigo-500/30 text-indigo-300' : 'bg-slate-700 text-slate-400'
                }`}>
                  {count}
                </span>
              )}
